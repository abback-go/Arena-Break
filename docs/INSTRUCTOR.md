...
| 6 | **ARENA BREAK ① 쏘고 죽인다** — 1장 전부 + 2-1 설계 논의 + 2-2 체력/데미지 |
| 7 | **ARENA BREAK ② 몰려오고 끝난다** — 2-3 적 AI부터 2장 끝까지 + 3장 + AI 코드 리뷰 |
| 8 | 중간고사 |
| 9 | 팀 기획 확정 + 팀 리포 세팅 |
| 10 | 기획 발표 |
| 11~13 | 팀 개발 |
...
**2-1을 2-2와 떼지 마세요.** 2-1은 `Health` 와 `IDamageable` 을 어떻게 나눌지 정하는
설계 논의이고, 2-2는 그 결정을 구현하는 절입니다. 순서가 바뀌면 학생은 자기가 정하지 않은
인터페이스를 받아쓰게 되고, 3장 3-1의 "이것이 2-1에서 인터페이스를 나눈 이유입니다" 가
근거를 잃습니다. 시간이 없으면 2-1을 줄이되(25→15분) 1일차 안에 둡니다.
