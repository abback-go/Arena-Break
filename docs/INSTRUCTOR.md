...
| 시점 | 확인 사항 | 미달 시 |
|---|---|---|
| 1주차 0:55 | 학생 80%가 MCP 연결 성공 | 실습 A를 10분 연장, 시연 B 축소 |
| 1주차 2:35 | 학생 70%가 이동 + 사격 동작 | `w1-complete` 태그 배포 후 진행 |
| 2주차 1:20 | 적이 플레이어를 추적함 | NavMesh 베이크 일괄 시연 |
| 2주차 2:35 | 웨이브가 넘어감 | `w2-complete` 태그 배포 |
| 3주차 2:10 | 코드 리뷰 세션 완료 | **이건 절대 생략 금지.** 빌드를 과제로 이전 |
